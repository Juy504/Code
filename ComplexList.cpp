...
void CopyNode(list head)//copy node
{
	Node* cur = head;
	while(cur !=NULL)
	{
...
void siblingNode(list head)//connect sibling pointer
{
	Node* cur = head;
	while(cur !=NULL)
	{
		...
		if(cur->sibling !=NULL)
			copyNode->sibling = cur->sibling->next;
		cur = cur->next;
	}
}

Node* realList(list head) //divild into two lists
{
	Node* cur = head;
	Node* copyHead = NULL;
	Node* copyNode = NULL;
...
Node* copyList(list head)
{
	CopyNode(head);//copy node
	siblingNode(head);//connect sibling pointer
	return realList(list head); //divild into two lists
}
int main()
{
	list l = NULL;
